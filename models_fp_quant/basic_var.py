...
from models_fp_quant.helpers import DropPath, drop_path
import pdb
...
class AdaLNSelfAttn(nn.Module):
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, step_idx, quant_KV, kv_bit):   # C: embed_dim, D: cond_dim
        # pdb.set_trace()
        if self.shared_aln:
        ...
        x_1 = self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1)
        x = x + self.drop_path(self.attn(x_1, attn_bias, self.block_idx, step_idx, quant_KV, kv_bit).mul_(gamma1))

        x_2 = self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2)
