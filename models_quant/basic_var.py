...
from models_quant.helpers import DropPath, drop_path
import pdb
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias, block_idx, step_idx):
        B, L, C = x.shape
...
class AdaLNSelfAttn(nn.Module):
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, step_idx):   # C: embed_dim, D: cond_dim
        # pdb.set_trace()
        ...
        x = x + self.drop_path(self.attn(self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), attn_bias, self.block_idx, step_idx).mul_(gamma1))
