...
def get_hadK(n, transpose=False):
    '''
    根据输入的整数 n 返回对应的 Hadamard 矩阵（如果存在的话）及其因子 K
    '''
    hadK, K = None, None
...
def matmul_hadU(X, transpose=False):
    # pdb.set_trace()
    n = X.shape[-1] # X是一个对角矩阵
    hadK, K = get_hadK(n, transpose)
    input = X.clone().view(-1, n, 1) # [4, 4, 1]
    output = input.clone() # [4, 4, 1]
    while input.shape[1] > K: # [4, 4, 1] , [4, 2, 2]
        input = input.view(input.shape[0], input.shape[1] // 2, 2, input.shape[2]) # [4, 2, 2, 1], [4, 1, 2, 2]
        output = output.view(input.shape)
        output[:, :, 0, :] = input[:, :, 0, :] + input[:, :, 1, :]
        output[:, :, 1, :] = input[:, :, 0, :] - input[:, :, 1, :]
        output = output.view(input.shape[0], input.shape[1], -1) # [4, 2, 2]
